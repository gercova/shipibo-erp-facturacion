...
class PayModeController extends Controller
{
    public function index() {
        return view('admin.paymodes.list');
    }

    public function get() {
        $paymodes     = PayMode::orderBy('id', 'DESC')->get();
        return Datatables()
                    ->of($paymodes)
                    ->addColumn('acciones', function($paymodes){
                        $id     = $paymodes->id;
    ...
    public function detail(Request $request)
    {
        ...
        $id       = $request->input('id');
        $paymode     = PayMode::where('id', $id)->first();
        return response()->json(['status'  => true, 'paymode' => $paymode]);
    }
